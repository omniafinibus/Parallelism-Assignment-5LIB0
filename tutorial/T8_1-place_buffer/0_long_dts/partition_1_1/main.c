...
int main ( void )
{
    ...
    while (1)  {
        ...
        if(error == 0){  
            ...
            gnow = read_global_timer();
            xil_printf("%04u/%010u: read token  ts=%010i data=%i,%i,%i,%i,%i dts=%010i\n", (uint32_t) (gnow >> 32), (uint32_t) gnow, (uint32_t)localToken.timestamp, localToken.data[0], localToken.data[1], localToken.data[2], localToken.data[3], localToken.data[4], (uint32_t)(gnow-localToken.timestamp));
        }
        else{
            // Throw error
            gnow = read_global_timer();
